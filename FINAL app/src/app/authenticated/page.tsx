...
const HeadsetItem = ({ id, name, image, onReturn }: { id: number; name: string; image: string, onReturn: () => void; }) => {
...
export default function Home() {
  ...
  const handleReturnSuccess = () => {
    ...
    if (userId) {
      // Re-run the same logic you had in useEffect
      fetch("/api/items", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        .catch((err) => console.error("Failed to fetch headsets:", err));
    }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {returnMessage && (
          <div className="bg-green-100 text-green-800 px-4 py-2 text-center font-medium">
            {returnMessage}
          </div>
        )}
